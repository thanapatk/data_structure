...
public class BST {
	...
	public void addLeftSubTreeToMin(BST subTree) {
		...
		Iterator currentTreeMinItr = this.findMin();
		int currentTreeMin = currentTreeMinItr != null ? ((TreeIterator) currentTreeMinItr).currentNode.data : Integer.MAX_VALUE;
		
		Iterator subTreeMaxItr = subTree.findMax();
		int subTreeMax = subTreeMaxItr != null ? ((TreeIterator) subTreeMaxItr).currentNode.data : Integer.MIN_VALUE;
		
		// won't be BST
		if (subTreeMax >= currentTreeMin) return;
		
		currentTreeMinItr = currentTreeMinItr != null ? currentTreeMinItr : new TreeIterator(this.root);
		
		// add to tree
		((TreeIterator) currentTreeMinItr).currentNode.left = subTree.root;
		subTree.root.parent = ((TreeIterator) currentTreeMinItr).currentNode;
		this.size += subTree.size;
	}
