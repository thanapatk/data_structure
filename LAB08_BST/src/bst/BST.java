...
public class BST {
	...
	public TreeIterator findMax(BSTNode n) throws Exception {
	...
	public void cloneTree(BST tt) {
		...
		while (nodes.size() != 0) {
			BSTNode currentNode = nodes.remove(0);
	...
	// not the best solution (works for small tree)
	public int findNextData(int n) {
		try {
			TreeIterator it = findMin(root);

			while (it.currentNode.data <= n)
				it.next();
